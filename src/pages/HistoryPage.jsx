...
import { Badge, Card, EmptyState, LoadingState, PageHeader, Select } from '../components/ui';
      ...
      <PageHeader
        ...
        action={
          <Select value={activeStage} onChange={(event) => setActiveStage(event.target.value)} className="min-w-[240px] bg-brutal-yellow">
            ...
            {rounds.map((round) => <option key={round.stage} value={round.stage}>{round.stage}</option>)}
          </Select>
        }
      />
            ...
            <Card key={round.stage} className="overflow-hidden">
              <div className="flex flex-col gap-2 border-b-4 border-brutal-black bg-brutal-yellow p-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-sm font-bold text-brutal-black/55">{round.guesses.length} palpite(s)</p>
                </div>
                <Badge tone="dark">{round.totalPoints} PONTOS</Badge>
              </div>
              <div className="divide-y-2 divide-brutal-black/10">
                {round.guesses.map((guess) => (
                  <div key={guess.id} className="grid gap-3 p-4 md:grid-cols-[1fr_auto_auto_auto] md:items-center">
                    <div>
                      <p className="font-display">{guess.match.homeTeam} x {guess.match.awayTeam}</p>
                      <p className="text-xs font-bold text-brutal-black/50">
                        {format(new Date(guess.match.matchDate), 'dd/MM HH:mm', { locale: ptBR })}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs font-bold text-brutal-black/50">Resultado</p>
                      <p className="font-display">
                        {guess.match.status === 'FINISHED' ? `${guess.match.homeScore} x ${guess.match.awayScore}` : 'Pendente'}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs font-bold text-brutal-black/50">Seu palpite</p>
                      <p className="font-display">{guess.homeGuess} x {guess.awayGuess}</p>
                    </div>
                    <Badge tone={guess.points === 3 ? 'success' : guess.points === 1 ? 'warning' : 'neutral'}>
                      {guess.points} PT
                    </Badge>
                  </div>
                ))}
